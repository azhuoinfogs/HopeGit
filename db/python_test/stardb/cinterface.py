...
class SdbField(ctypes.Structure):
...
class CTaosInterface(object):
    ...
    libtaos = ctypes.CDLL('libstardb.so')

    libtaos.stardb_fetch_fields.restype = ctypes.POINTER(SdbField)
    libtaos.stardb_init.restype = None
    libtaos.stardb_connect.restype = ctypes.c_void_p
    libtaos.stardb_get_result.restype = ctypes.c_void_p
    libtaos.stardb_decode_result.restype = ctypes.POINTER(ctypes.c_void_p)
    libtaos.stardb_errstr.restype = ctypes.c_char_p
    ...
    def __init__(self, config=None):
        ...
        CTaosInterface.libtaos.stardb_init()
    ...
    def close(connection):
        ...
        '''
        CTaosInterface.libtaos.stardb_close(connection)
    ...
    def query(connection, sql):
        ...
        '''
        try:
            return CTaosInterface.libtaos.stardb_query(connection, ctypes.c_char_p(sql.encode('utf-8')))
        except AttributeError:
    ...
    def affectedRows(connection):
        ...
        """
        return CTaosInterface.libtaos.stardb_affected_rows(connection)

    @staticmethod
    def useResult(connection):
        '''Use result after calling self.query
        '''
        result = ctypes.c_void_p(CTaosInterface.libtaos.stardb_get_result(connection))
        fields = []
    ...
    def fetchBlock(result, fields):
        ...
        num_of_rows = CTaosInterface.libtaos.stardb_fetch_block(
            result, ctypes.byref(pblock))

        if num_of_rows == 0:
            return None, 0

        isMicro = (CTaosInterface.libtaos.stardb_result_precision(result) == FieldType.C_TIMESTAMP_MICRO)
    ...
    def freeResult(result):
        CTaosInterface.libtaos.stardb_free_result(result)
        result.value = None

    @staticmethod
    def fieldsCount(connection):
        return CTaosInterface.libtaos.stardb_field_count(connection)

    @staticmethod
    def fetchFields(result):
        return CTaosInterface.libtaos.stardb_fetch_fields(result)
    ...
    def errno(connection):
        ...
        """
        return CTaosInterface.libtaos.stardb_errno(connection)

    @staticmethod
    def errStr(connection):
        """Return the error styring
        """
        return CTaosInterface.libtaos.stardb_errstr(connection).decode('utf-8')
