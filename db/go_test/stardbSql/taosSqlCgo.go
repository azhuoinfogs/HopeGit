...
func (mc *taosConn) taosConnect(ip, user, pass, db string, port int) (taos unsafe.Pointer, err error) {
	...
	taosObj := C.stardb_connect(cip, cuser, cpass, cdb, (C.ushort)(port))
	if taosObj == nil {
...
func (mc *taosConn) taosQuery(sqlstr string) (int, error) {
	...
	code := int(C.stardb_query(mc.taos, csqlstr))

	if 0 != code {
		mc.taos_error()
		errStr := C.GoString(C.stardb_errstr(mc.taos))
	...
	num_fields := int(C.stardb_field_count(mc.taos))
	if 0 == num_fields { // there are no select and show kinds of commands
		mc.affectedRows = int(C.stardb_affected_rows(mc.taos))
		mc.insertId = 0
	}
...
func (mc *taosConn) taos_close() {
	C.stardb_close(mc.taos)
}

func (mc *taosConn) taos_error() {
	// free local resouce: allocated memory/metric-meta refcnt
	//var pRes unsafe.Pointer
	pRes := C.stardb_get_result(mc.taos)
	C.stardb_free_result(pRes)
}
