...
func (mc *taosConn) readColumns(count int) ([]taosSqlField, error) {
	...
	result = C.stardb_get_result(mc.taos)
	if result == nil {
		return nil, errors.New("invalid result")
	}

	pFields := (*C.struct_sdbField)(C.stardb_fetch_fields(result))

	// TODO: Optimized rewriting !!!!
	fields := (*[1 << 30]C.struct_sdbField)(unsafe.Pointer(pFields))
...
func (rows *taosSqlRows) readRow(dest []driver.Value) error {
	...
	result = C.stardb_get_result(mc.taos)
	if result == nil {
		return errors.New(C.GoString(C.stardb_errstr(mc.taos)))
	}

	//var row *unsafe.Pointer
	//row := C.taos_fetch_row(result)
	row := C.stardb_decode_result(result)
	if row == nil {
		rows.rs.done = true
		C.stardb_free_result(result)
		rows.mc = nil
		return io.EOF
	}
	...
	for i := range dest {
		...
		case C.TSDB_DATA_TYPE_TIMESTAMP:
			if mc.cfg.parseTime == true {
				...
				dest[i] = timestampConvertToString(timestamp, int(C.stardb_result_precision(result)))
			} else {
				dest[i] = (int64)(*((*int64)(currentRow)))
			}
			break
