...
spec_file="${script_dir}/starengine.spec"
...
for dirlist in $(ls ${cur_dir}); do
  ...
  if test -e ${dirlist}; then
    cp ${cur_dir}/${dirlist} ${output_dir}/Starengine-${starengine_ver}.rpm
  fi
done
}
...
${csudo} rpmbuild --define="_version ${starengine_ver}" --define="_topdir ${pkg_dir}" --define="_compiledir ${compile_dir}" -bb ${spec_file}
...
if [ "$verMode" == "cluster" ]; then
  rpmname="Starengine-server-"${starengine_ver}-${osType}-${cpuType}
...
mv ${output_dir}/Starengine-${starengine_ver}.rpm ${output_dir}/${rpmname}
