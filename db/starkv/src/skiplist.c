/*
 * Copyright (c) 2020 bj-Starbalze, Inc. <qiang.hu@starblaze-tech.com>
 *
...
static void skipListDoInsert(SSkipList *pSkipList, SSkipListNode **forward, SSkipListNode *pNode);
static SSkipListNode* skipListPushBack(SSkipList *pSkipList, SSkipListNode *pNode);
static SSkipListNode* skipListPushFront(SSkipList* pSkipList, SSkipListNode *pNode);
static SSkipListIterator* doCreateSkipListIterator(SSkipList *pSkipList, int32_t order);


// when order is STAR_ORDER_ASC, return the last node with key less than val
// when order is STAR_ORDER_DESC, return the first node with key large than val
...
SSkipList *skipListCreate(uint8_t maxLevel, uint8_t keyType, uint8_t keyLen, uint8_t dupKey, uint8_t lock,
...
void *skipListDestroy(SSkipList *pSkipList) {
  if (pSkipList == NULL) {
    return NULL;
  }
...
void skipListNewNodeInfo(SSkipList *pSkipList, int32_t *level, int32_t *headSize) {
  if (pSkipList == NULL) {
    *level = 1;
...
SSkipListNode *skipListPut(SSkipList *pSkipList, SSkipListNode *pNode) {
  ...
  if (pSkipList->size == 0 || pSkipList->comparFn(SL_GET_SL_MAX_KEY(pSkipList), newDatakey) < 0) {
    return skipListPushBack(pSkipList, pNode);
  }
  ...
  if (pSkipList->comparFn(newDatakey, minKey) < 0) {
    return skipListPushFront(pSkipList, pNode);
  }
  ...
  skipListDoInsert(pSkipList, forward, pNode);
  return pNode;
}



SArray* skipListGet(SSkipList *pSkipList, SSkipListKey key) {
  SArray* sa = starArrayInit(1, POINTER_BYTES);

  if (pSkipList->lock) {
    pthread_rwlock_wrlock(pSkipList->lock);
  }

  SSkipListNode* pNode = getPriorNode(pSkipList, key, STAR_ORDER_ASC);
  while (1) {
...
size_t skipListGetSize(const SSkipList* pSkipList) {
  if (pSkipList == NULL) {
    return 0;
  }
...
// static int32_t skipListEndParQuery(SSkipList *pSkipList, SSkipListNode *pStartNode, SSkipListKey *pEndKey,
...
//      p = p->pForward[0];
//    } else if (ret == 0) {
//      if (cond == STAR_RELATION_LESS_EQUAL) {
//        numOfRes++;
//        p = p->pForward[0];
//      } else {
//        break;
...
// */
// int32_t skipListGets(SSkipList *pSkipList, SSkipListKey *pKey, SSkipListNode ***pRes) {
//  (*pRes) = NULL;
//
//  SSkipListNode *pNode = skipListGet(pSkipList, pKey);
//  if (pNode == NULL) {
//    return 0;
//  }
//
...
//  }
//
//  return skipListEndParQuery(pSkipList, pPrev->pForward[0], &pNode->key, STAR_RELATION_LESS_EQUAL, pRes);
//}
//
// static SSkipListNode *skipListParQuery(SSkipList *pSkipList, SSkipListKey *pKey, int32_t cond) {
...
//  pthread_rwlock_rdlock(&pSkipList->lock);
//
//  if (cond == STAR_RELATION_GREATER_EQUAL || cond == STAR_RELATION_GREATER) {
...
//
//    // backward check if previous nodes are with the same value.
//    if (cond == STAR_RELATION_GREATER_EQUAL && ret == 0) {
...
//    }
//
//    if (ret > 0 || cond == STAR_RELATION_GREATER_EQUAL) {
//      pthread_rwlock_unlock(&pSkipList->lock);
//      return x->pForward[0];
//    } else {  // cond == STAR_RELATION_GREATER && ret == 0
...
uint32_t skipListRemove(SSkipList *pSkipList, SSkipListKey key) {
  uint32_t count = 0;
...
void skipListRemoveNode(SSkipList *pSkipList, SSkipListNode *pNode) {
...
SSkipListIterator* skipListCreateIter(SSkipList *pSkipList) {
  if (pSkipList == NULL) {
    return NULL;
  }
  
  return doCreateSkipListIterator(pSkipList, STAR_ORDER_ASC);
}

SSkipListIterator *skipListCreateIterFromVal(SSkipList* pSkipList, const char* val, int32_t type, int32_t order) {
  assert(order == STAR_ORDER_ASC || order == STAR_ORDER_DESC);
...
bool skipListIterNext(SSkipListIterator *iter) {
  ...
  if (iter->order == STAR_ORDER_ASC) {  // ascending order iterate
  ...
  iter->step += 1;
  return (iter->order == STAR_ORDER_ASC)? (iter->cur != pSkipList->pTail) : (iter->cur != pSkipList->pHead);
}

SSkipListNode *skipListIterGet(SSkipListIterator *iter) {
...
void* skipListDestroyIter(SSkipListIterator* iter) {
  if (iter == NULL) {
    return NULL;
  }
...
// bool skipListRemove(SSkipList *pSkipList, SSkipListKey *pKey) {
//  SSkipListNode *forward[MAX_SKIP_LIST_LEVEL] = {0};
//  __compar_fn_t  filterComparFn = getComparFunc(pSkipList, pKey->nType);
//
//  pthread_rwlock_rdlock(&pSkipList->lock);
...
void skipListPrint(SSkipList *pSkipList, int16_t nlevel) {
...
void skipListDoInsert(SSkipList *pSkipList, SSkipListNode **forward, SSkipListNode *pNode) {
...
SSkipListNode* skipListPushFront(SSkipList* pSkipList, SSkipListNode *pNode) {
  SSkipListNode* forward[MAX_SKIP_LIST_LEVEL] = {0};
...
SSkipListNode* skipListPushBack(SSkipList *pSkipList, SSkipListNode *pNode) {
