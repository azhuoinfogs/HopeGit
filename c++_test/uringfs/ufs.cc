...
static const string DBPATH = "/opt/boss_meta";       // field -> value
static const string LOGPATH = "/var/log";       // field -> value
...
class Inode {
	...
	Inode(Metadata &meta) {
		ino = meta.inode_id;
		name = meta.name;
		is_dir_ = meta.is_directory;
		ctime = time(NULL);
		mtime = time(NULL);
		atime = time(NULL);
		mode = meta.is_directory ? S_IFDIR | 0755 : S_IFREG | 0644;
		nlink = meta.nlink;
		uid = meta.owner_id;
		gid = meta.group_id;
	}
...
class Inodes {
	...
	std::mutex mutex;
    // std::shared_ptr<BossMeta> boss_meta;
	...
	Inodes()
	{
        ...
        // boss_meta = std::make_shared<BossMeta>("/opt/boss_meta/");
	}
    ...
    std::unique_ptr<Inode> find_meta_locked(fuse_ino_t ino)
	{
        struct Metadata metadata;
        if (!BossMeta::Instance().getMetadataByInode(ino, &metadata)) {
            memfs_panic("Inode not found");
            return nullptr;
        }
		return std::make_unique<Inode>(metadata);
	}
	std::unique_ptr<Inode> find1(fuse_ino_t ino)
	{
		std::shared_lock lock(inodes_mutex);
		return find_meta_locked(ino);
	}
	...
	Inode *create1(uint64_t parentInodeId, const std::string &name, bool is_dir, mode_t mode)
	{
		std::unique_lock<std::shared_mutex> lock(inodes_mutex);
		if (!BossMeta::Instance().createEntry(parentInodeId, name, is_dir))
		{
			memfs_panic("Inode not found");
			return nullptr;
		}
...
Dentry *Inode::find_child_locked(const std::string &name) const
{
	...
	Metadata metadata;
	if (!BossMeta::Instance().getMetadataByPath(name, &metadata))
	{
		memfs_panic("Inode not found");
		return nullptr;
	}
			  ...
			  struct fuse_file_info *fi)
{
	...
	auto inode_data = Inodes.find1(actual_ino);
	if (!inode_data) {
		fuse_reply_err(req, ENOENT);
		return;
	}
...
{
	auto parentInode = Inodes.find1(parent);
...
static void InitLog(bool foreground, const string& prefix)
{
    CLog::SLogContext  logContext;
    logContext.log_level = LOG_PRI_WARN;
    logContext.log_color = LOG_CLR_PID;
    logContext.b_log_rotate = true;

    string logFileName = prefix + "/boss_meta.log";
    DIR* dir = opendir(prefix.c_str());
    if(!dir) {
        mkdir(prefix.c_str(), 0775);
    }
    closedir(dir);
    strcpy(logContext.log_file_name, logFileName.c_str());

    if(foreground) {
        logContext.b_log_screen = true; // write to console  or not
    } else {
        logContext.b_log_screen = false; // write to console  or not
    }
    CLog::getInstance().setContext(&logContext);
    chdir(prefix.c_str());
}
int main(int argc, char *argv[])
{
	...
	int ret = -1;
    InitLog(false, LOGPATH);

	struct fuse_loop_config *config = new fuse_loop_config();
	if (config == NULL) {
